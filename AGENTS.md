...
- Nest `<Errored>` *inside* `<Loading>`, never outside. A read that rejects
...
- Never write an effect whose only job is resetting state when a key changes.
  `createSignal(fn)` is a *writable memo*: it recomputes (and so resets) when
...
- A DOM node an effect depends on belongs in a signal, not a `ref` variable.
  A plain `let` is invisible to the effect, so it cannot re-run once the node
  exists.
