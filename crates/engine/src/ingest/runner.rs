...
//!
//! Ordering is the whole design. Search reads only the eight serving tables
//! (`search::schema_guard::REQUIRED_TABLES`); ingest and merge write only
//! staging tables. So nothing this runner does is observable by a search
//! request until `materialize_serving` runs, and the gate sits before the
//! merge, which is the last point where rejecting a bad file costs nothing:
//! `merge_core.sql` upserts `document_attribute.full_name` in place and cannot
//! restore what it overwrote.
//!
//! That staging/serving split is why no working copy of contacts.sqlite is
//! made. A copy would buy isolation the schema already provides, at two full
//! rewrites of a 37 GB file per job.
...
/// Pipeline tuning applied to every job this engine runs. Fixed at startup:
/// these are properties of the host the engine sits on, not of the file being
/// ingested, so a client cannot set them per request.
...
/// Reason a job stopped short of `complete`. Gate rejection is not an error in
/// the operational sense (the file was judged and refused), so it carries the
/// checks that explain the decision.
pub enum JobFailure {
...
pub fn run(store: &JobStore, request: &JobRequest) -> Result<(), JobFailure> {
    ...
    if !gate.passed {
        // Nothing merged yet, so discarding the shards is a complete rollback.
...
fn stage(store: &JobStore, request: &JobRequest) -> Result<IngestSession, JobFailure> {
    ...
    let session = ingest_to_shards(ShardIngestConfig {
        ...
        // The job id doubles as the run id so shard directories on disk are
        // traceable back to a job record.
...
fn merge(store: &JobStore, request: &JobRequest, session: IngestSession) -> Result<(), JobFailure> {
    ...
    // merge_ingest_session consumes the session, so the run directory is
    // captured before the call and removed on both paths.
...
fn materialize(store: &JobStore, request: &JobRequest, snapshot_id: i64) -> Result<(), JobFailure> {
    ...
    // The first step whose output a search request can see. It commits in
    // chunks, so an interrupted run leaves rows consistent and the remaining
    // dirty ids queued for the next call.
...
/// Shard databases are scratch and can be large. Nothing in the pipeline
/// deletes them, so a long-lived process must, or they accumulate next to
/// contacts.sqlite on every job.
