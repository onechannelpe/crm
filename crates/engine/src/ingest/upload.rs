//! Two-phase streamed upload: reserve metadata first, then stream the blob
//! directly to disk while hashing it.
...
// Active uploads remain reserved until their request finishes.
...
// Registry and queue state are in-memory, so stale files cannot resume.
...
// Enforce the registered size even when Content-Length is absent.
async fn stream_to_hashed_file(
    ...
    if written != limit {
        return Err(io::Error::new(
            ...
            format!("upload size {written} does not match declared size_bytes {limit}"),
        ));
    }
...
mod tests {
    ...
    fn mapping() -> SourceMapping {
        embedded::mapping_for("osiptel_scan_sunat").expect("embedded mapping")
    }
    ...
    fn expired_reservation_cannot_be_claimed() {
        ...
        registry
            ...
            .get_mut(&upload_id)
            .expect("entry")
            .reserved_at = Instant::now() - UPLOAD_RESERVATION_TTL - Duration::from_secs(1);
    ...
    #[test]
    fn claimed_reservations_do_not_expire() {
        let registry = UploadRegistry::new(1);
