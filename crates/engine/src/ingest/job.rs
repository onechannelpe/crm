...
//!
//! Jobs live in their own SQLite file rather than in contacts.sqlite or
//! leads.sqlite. contacts.sqlite is replaced wholesale by the offline
//! `pipeline promote`, which would take job history with it; leads.sqlite
//! belongs to a different domain. A dedicated file keeps one owner for this
//! state and survives both.
//!
//! State is durable rather than in-memory because a job outlives a request by
//! minutes and must still be answerable after a deploy restarts the process.
...
/// Steps a job moves through, in order. A job that fails records the step it
/// was in, so `failed` is a flag on the record rather than a state here.
...
pub struct JobRecord {
    ...
    /// Serialized `GateResult`, present once the gate has run.
...
impl JobStore {
    ...
    /// Counts jobs not yet finished (queued or actively running). Used
    /// alongside `UploadRegistry::pending_count` to bound how many uploaded
    /// files can occupy scratch disk at once, across both phases.
    ...
    /// Records failure without moving `step`, so the record says which step the
    /// job died in.
    ...
    /// Marks jobs left `running` by a process that died as failed. Nothing
    /// resumes them: a job holds no cross-restart state, and any staging rows a
    /// half-finished merge wrote are still flagged in the dirty tables, so the
    /// next materialize picks them up.
...
fn create_schema(conn: &Connection) -> Result<(), StartupError> {
    conn.execute_batch(
        ...
        CREATE INDEX IF NOT EXISTS ingest_job_outcome ON ingest_job(outcome, created_at DESC);",
    )
