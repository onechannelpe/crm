...
pub struct RegisterUploadRequest {
    /// Must name a mapping compiled into the binary
    /// (`pipeline::config::embedded`).
    pub source_key: String,
    /// Identifies this delivery of the source. Re-posting the same label
    /// replaces that snapshot rather than adding another.
    pub snapshot_label: String,
    pub snapshot_date: String,
    /// Declared upfront so the blob step can be rejected mid-stream the
    /// moment it's exceeded, rather than discovered only after a full,
    /// wasted transfer.
    pub size_bytes: u64,
    /// Lowercase hex SHA-256 of the file, checked against what actually
    /// streams in during the blob step.
    pub sha256: String,
}
