//! End-to-end ingest against a real pipeline-schema database.
//!
//! The point of these tests is the ordering claim the design rests on: a
//! rejected file must leave the serving tables untouched, and an accepted one
//! must reach them. They also cover the two-phase upload protocol itself:
//! registering reserves a slot and validates fast, the blob step is what
//! actually moves bytes and enqueues a job.
...
impl Harness {
    ...
    /// Registers then uploads in one call, returning the blob step's
    /// response: that is the call that actually creates the job.
    ...
    /// Jobs run on a blocking thread, so the outcome is polled rather than
    /// awaited.
...
async fn ingests_a_sunat_scan_file_into_the_serving_projection() {
    ...
    // The RUC10 is the field this source exists to contribute: it is derived
    // from the `doc` column, not present as its own column in the CSV.
    assert_eq!(
...
/// A file whose header does not match the mapping resolves no columns, so every
/// row normalizes to nothing. The gate must catch that before the merge writes
/// anything.
#[tokio::test]
...
async fn a_checksum_mismatch_is_rejected_and_frees_the_queue_slot() {
    ...
    let register = harness
        .register(serde_json::json!({
            ...
            // Well-formed but wrong: does not hash to `contents`.
    ...
    // The slot the mismatched upload held must be freed, not leaked: a
    // second registration at the same (queue depth 1) capacity must succeed.
    let retry = harness
...
async fn actual_bytes_differing_from_the_declared_size_are_rejected() {
    ...
    // Uploads a longer body than what was declared at registration.
    let actual = format!(
...
async fn a_full_queue_is_rejected_with_service_unavailable() {
    ...
    // The first reservation is still unclaimed, so it still occupies the
    // queue's only slot.
    let second = harness
