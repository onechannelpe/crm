...
/// Creates a seeded in-memory-style temp SQLite file with the full search schema.
#[allow(dead_code)]
pub fn create_test_db() -> tempfile::NamedTempFile {
    ...
    conn.execute_batch(
        ...
        INSERT INTO search_projection(
            id,dni,name,birth_date,sex,org_ruc,org_name,trade_name,org_status,fiscal_address,
            role_name,role_start_date,rep_doc_type,rep_doc_number,rep_name,phone_primary,phone_secondary
        ) VALUES
            (1,'12345678','JUAN PEREZ','1980-05-10','M','20100011111','ACME SAC','ACME','ACTIVO','AV. LIMA 123',
             'GERENTE GENERAL','2020-01-01','','','','999111222','999333444'),
            (2,'87654321','MARIA LOPEZ',NULL,'F','20100011111','ACME SAC','ACME','ACTIVO','AV. LIMA 123',
             'SOCIO','2020-01-01','','','','988777666',NULL),
            (3,'11223344','CARLOS DIAZ',NULL,NULL,'','',NULL,NULL,NULL,
             NULL,NULL,'','','','977000111',NULL);
...
/// Opens a read-only pool over the temp file.
#[allow(dead_code)]
pub fn test_pool(file: &tempfile::NamedTempFile) -> engine_infra::sqlite::SqlitePool {
    make_readonly_pool(file.path().to_str().expect("path")).expect("pool")
}

/// Signs a request body with the given secret, returning (timestamp, hex-signature).
#[allow(dead_code)]
