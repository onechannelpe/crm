...
use search_service::contracts::{SearchRequest, SearchType};
...
fn perf_regression_probe() {
    let db_path =
        std::env::var("ENGINE_CONTACTS_DB_PATH").expect("ENGINE_CONTACTS_DB_PATH required");
    ...
    let pool = engine_infra::sqlite::make_readonly_pool(&db_path).expect("pool");
    ...
    let payload = json!({ "iterations": iterations, "metrics": metrics });
...
fn bench(
    ...
    json!({"iterations":iterations,"hits":hits,"p50_ms":p50,"p95_ms":p95,"avg_ms":total/iterations as f64,"qps":iterations as f64/(total/1000.0)})
}
