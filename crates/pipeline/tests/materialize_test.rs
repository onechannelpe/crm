...
/// One document with a name, a natural RUC, and two phones of unequal
/// confidence, plus the dirty marker that makes it eligible.
...
/// The ranking CTEs used to scan document_phone in full, so a dirty document
/// could pick up ordering influenced by rows belonging to other documents.
/// Rebuilding one document must leave every other projection row untouched.
#[test]
...
/// The FTS index is keyed by rowid = doc_id, so a rebuild that failed to delete
/// the old entry would return the document twice, or return a stale name.
#[test]
...
/// A phone belonging to a document outside the dirty set must not leak into
/// the ranking. This is the failure the unscoped ranking CTEs made possible.
#[test]
