...
pub fn map_snapshot_only(mapping_path: &str, input_path: &str) -> Result<usize, PipelineError> {
    ...
    let mut reader = mapping.reader(input_path)?;
...
pub(super) fn dispatch_records(
    ...
    for (index, result) in reader.byte_records().enumerate() {
        let byte_record = result?;
        let record = mapping.decode_byte_record(&byte_record)?;
        let source_row_number = (index + 1) as i64;

        // Bounds resource use for untrusted uploads; the offline caller passes
        // i64::MAX since local files are already trusted.
        ...
        let worker_index = index % workers;
