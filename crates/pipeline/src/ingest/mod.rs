...
//!
//! Turns one source snapshot into shard databases and tracks snapshot progress
...
pub fn ingest_to_shards(config: ShardIngestConfig<'_>) -> Result<IngestSession, PipelineError> {
    ...
    let run_root = run_root(db_path, run_id);
    ...
    for handle in handles {
        worker_results.push(join_worker(handle, db_path, snapshot_id)?);
    }
...
fn run_root(db_path: &str, run_id: &str) -> PathBuf {
    Path::new(db_path)
        .parent()
