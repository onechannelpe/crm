...
export function createSearchAllowanceService(repos: Repositories) {
  ...
  async function ensureLedger(userId: number) {
    const policy = await policyService.getEffectiveSearchPolicy(userId);
  ...
  return {
    ...
    async getCurrentSearchAllowance(userId: number) {
      const { ledger, policy } = await ensureLedger(userId);
      return {
