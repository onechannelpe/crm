...
actor-keyed telemetry row. Once authentication succeeds, authorization, step-up,
and business failures are recorded because the denied attempt can be attributed
to a known actor.
...
`runAction` normally requires. These endpoints skip identity and actor
telemetry, but still share the same fault projection rules. Domain auth events
and auth analytics remain owned by the auth flow itself.
