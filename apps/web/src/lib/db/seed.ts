...
export async function seedIfEmpty() {
    ...
    await db
        ...
        .values([
            ...
            { ruc: "20384577831", name: "Constructora Sur del Perú S.A.C.", created_at: now },
            { ruc: "20600188214", name: "Retail Andino S.R.L.", created_at: now },
            { ruc: "20566100911", name: "Servicios Financieros del Norte S.A.", created_at: now },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { organization_id: 6, dni: "41900452", name: "Alberto Yauri Flores", phone_primary: "+51 988 000 111", created_at: now },
            { organization_id: 6, dni: "42234009", name: "Diana Cáceres Lazo", phone_primary: "+51 988 000 112", created_at: now },
            { organization_id: 7, dni: "70654312", name: "José Luis Huamán", phone_primary: "+51 988 000 113", created_at: now },
            { organization_id: 8, dni: "49123407", name: "Luisa Acurio Pérez", phone_primary: "+51 988 000 114", created_at: now },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { branch_id: 2, email: "north-supervisor@crm.local", password_hash: passwordHash, full_name: "North Supervisor", role: "supervisor", is_active: 1, created_at: now },
            { branch_id: 2, email: "north-exec@crm.local", password_hash: passwordHash, full_name: "North Executive", role: "executive", is_active: 1, created_at: now },
            { branch_id: 2, email: "north-backoffice@crm.local", password_hash: passwordHash, full_name: "North Back-Office", role: "back_office", is_active: 1, created_at: now },
            { branch_id: 2, email: "north-logistics@crm.local", password_hash: passwordHash, full_name: "North Logistics", role: "logistics", is_active: 1, created_at: now },
            { branch_id: 1, email: "manager@crm.local", password_hash: passwordHash, full_name: "Sales Manager Lima", role: "sales_manager", is_active: 1, created_at: now },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { user_id: 9, contact_id: 13, assigned_at: now, expires_at: now + sevenDays, status: "active" },
            { user_id: 9, contact_id: 14, assigned_at: now - oneDay, expires_at: now + sevenDays, status: "active" },
            { user_id: 9, contact_id: 15, assigned_at: now - oneDay * 2, expires_at: now + sevenDays, status: "active" },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { contact_id: 13, user_id: 9, status: "draft", created_at: now - oneDay, updated_at: now - oneDay },
            { contact_id: 14, user_id: 9, status: "rejected", created_at: now - oneDay * 4, updated_at: now - oneDay * 2 },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { product_id: 1, serial_number: "MOV-2026-0101", status: "available", created_at: now },
            { product_id: 2, serial_number: "FIB-2026-0101", status: "available", created_at: now },
            { product_id: 2, serial_number: "FIB-2026-0102", status: "reserved", created_at: now },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            { inventory_item_id: 3, charge_note_id: 2, locked_at: now - oneDay, expires_at: now + oneDay },
            { inventory_item_id: 11, charge_note_id: 6, locked_at: now - oneDay, expires_at: now + oneDay },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
    ...
    await db
        ...
        .values([
            ...
            { contact_id: 13, user_id: 9, outcome: "Pending confirmation", notes: "Requested technical fact sheet", duration_seconds: 210, created_at: now - oneDay },
            { contact_id: 14, user_id: 9, outcome: "Rejected by back office", notes: "Need clearer DNI image", duration_seconds: 180, created_at: now - oneDay * 2 },
        ])
        .onConflict((oc) => oc.doNothing())
        .execute();
