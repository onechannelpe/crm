...
export default function NewSalePage() {
    ...
    const [inventory, { refetch: refetchInventory }] = createResource(getAvailableInventory);
    const [draft, { refetch: refetchDraft }] = createResource(noteId, getSaleDraftContext);
    ...
    async function handleAddItem() {
        ...
        try {
            await addSaleItem(currentNoteId, Number(selectedProductId()), Number(quantity()));
    ...
    async function handleAddDocument() {
        ...
        try {
            ...
            showToast("error", getErrorMessage(err, "No se pudo registrar documento"));
        }
    }
    ...
    async function handleLockInventory() {
        ...
        try {
            ...
            showToast("error", getErrorMessage(err, "No se pudo reservar inventario"));
        }
    }
            ...
            <Show when={noteId()}>
                <Card>
                    <div class="p-6 space-y-6">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">Nota de cargo #{noteId()}</h3>
                            <p class="text-sm text-gray-500 mt-1">Debe tener productos, documentos y equipo bloqueado.</p>
                        </div>
                        ...
                        <div class="grid gap-4 md:grid-cols-3">
                            <div class="space-y-2 rounded border p-3">
                                ...
                                <select
                                    ...
                                    onInput={(e) => setSelectedProductId((e.target as HTMLSelectElement).value)}
                                >
                                ...
                                <Input type="number" label="Cantidad" value={quantity()} min="1" onInput={(e) => setQuantity((e.target as HTMLInputElement).value)} />
                                <Button onClick={handleAddItem}>Agregar</Button>
                            </div>

                            <div class="space-y-2 rounded border p-3">
                                <p class="text-sm font-medium">Documento manual</p>
                                <Input label="Archivo" value={docName()} onInput={(e) => setDocName((e.target as HTMLInputElement).value)} />
                                <Input label="Tipo MIME" value={docType()} onInput={(e) => setDocType((e.target as HTMLInputElement).value)} />
                                <Input type="number" label="Tamaño (KB)" value={docSizeKb()} onInput={(e) => setDocSizeKb((e.target as HTMLInputElement).value)} />
                            ...
                            <div class="space-y-2 rounded border p-3">
                                ...
                                <select
                                    ...
                                    onInput={(e) => setSelectedInventoryId((e.target as HTMLSelectElement).value)}
                                >
                        ...
                        <Show when={draft()}>
                            {(ctx) => (
                                <div class="rounded border p-3 text-sm space-y-1">
                                    ...
                                    <p>
                                        Inventario bloqueado: {ctx().inventoryLock?.serial_number ?? "No"}
                                    </p>
                                </div>
                            )}
                        </Show>
