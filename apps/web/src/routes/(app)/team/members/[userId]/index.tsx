...
export default function TeamMemberCapacityPage() {
  ...
  createEffect(() => {
    ...
    if (!snapshot) return;
    setSearchLimit(String(snapshot.searchStatus.monthlySearchLimit));
    setBufferTarget(String(snapshot.leadStatus.activeBufferTarget));
    setDailyRefillLimit(String(snapshot.leadStatus.dailyRefillLimit));
  });
