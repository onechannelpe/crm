...
import { requestDownloadToken } from "~/server/files/service";
...
export async function POST(event: Pick<APIEvent, "params">): Promise<Response> {
  try {
    ...
    const result = await requestDownloadToken(ctx, safeId, {
      repo,
      storage,
      syncExecutor,
    });
    ...
    return Response.json({ token: result.value.token }, { status: 201 });
  } catch (err) {
    return new Response(
