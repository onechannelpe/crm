...
describe("sales audit contracts", () => {
    ...
    it("persists stage transition payloads for submit and approve", async () => {
        ...
        const submitted = logs.find((log) => log.action === "charge_note_submitted");
    ...
    it("persists reject payload fields and resubmission transition", async () => {
        ...
        expect(parseChanges(rejectLog?.changes ?? null).fields).toEqual(["dni_file"]);

        const submittedLogs = logs.filter((log) => log.action === "charge_note_submitted");
        const resubmission = submittedLogs.find((log) => parseChanges(log.changes).from === "rejected");
        expect(resubmission).toBeDefined();
        expect(parseChanges(resubmission?.changes ?? null).to).toBe("pending_review");
    });
});
