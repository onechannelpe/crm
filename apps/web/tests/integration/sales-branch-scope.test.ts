...
describe("sales branch scoping", () => {
    it("returns pending notes only for the requested branch", async () => {
        const ctx = await createIsolatedTestDb("branch-queue");
        try {
            ...
            const limaRows = await ctx.repos.chargeNotes.findPendingReviewWithContactsByBranch(1);
            const norteRows = await ctx.repos.chargeNotes.findPendingReviewWithContactsByBranch(2);
    ...
    it("allows cross-branch approval only with superuser bypass", async () => {
        const ctx = await createIsolatedTestDb("branch-bypass");
        try {
            const noteLima = await ctx.repos.chargeNotes.create(1, 1);
            await ctx.repos.chargeNotes.updateStatus(noteLima, "pending_review");
            ...
            await ctx.repos.documents.create({
                ...
                filename: "dni.pdf",
                filepath: `uploads/${noteLima}/dni.pdf`,
