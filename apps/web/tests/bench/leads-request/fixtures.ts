...
export async function seedLeadsRequestFixtures(
  ...
  const users = Array.from({ length: USER_POOL_SIZE }, (_, index) => ({
    ...
    password_hash: "hash",
    full_name: `Bench Leads ${USER_ID_START + index}`,
  ...
  const engineClient: EngineClient = {
    ...
    async search(_type, value) {
      return {
        ...
        results: [
          {
            person: { dni: `bench-${value}`, name: `Bench Contact ${value}`, birth_date: null, birth_place: null, sex: null, marital_status: null, location_text: null, ubigeo_code: null, mother_name: null, father_name: null, email: null, ruc: null },
            org: { ruc: value, name: `Bench Org ${value}`, trade_name: null, company_type: null, status: null, condition: null, fiscal_address: null, registration_date: null, activity_start_date: null, line_of_business: null, economic_activity: null },
            role: null,
