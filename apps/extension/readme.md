...
The handoff starts in the web application. The web application creates a signed handoff token through [`../web/src/routes/api/extension/handoff-token.ts`](../web/src/routes/api/extension/handoff-token.ts) and sends `assignment.handoff` through [`../web/src/lib/extension/runtime.ts`](../web/src/lib/extension/runtime.ts). The extension verifies sender origin and token signature in [`src/services/external-auth.ts`](src/services/external-auth.ts), then claims a session through [`../web/src/routes/api/extension/session/claim.ts`](../web/src/routes/api/extension/session/claim.ts). Queued extension events are posted to [`../web/src/routes/api/extension/events.ts`](../web/src/routes/api/extension/events.ts). Session refresh uses [`../web/src/routes/api/extension/session/refresh.ts`](../web/src/routes/api/extension/session/refresh.ts).
